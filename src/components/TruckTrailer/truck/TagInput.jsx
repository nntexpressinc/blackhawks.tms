...
const TagInput = ({ truckData, handleChange }) => {
  ...
  useEffect(() => {
    const fetchTags = async () => {
      try {
        const response = await ApiService.getData('/truck/tags/');
        setTags(response);
      } catch (error) {
  ...
  const handleAddTag = async (e) => {
    if (e.key === 'Enter') {
      try {
        const response = await ApiService.postData('/truck/tags/', { tag: newTag });
